...
> 차량의 리니어·가속도·인버터/BMS·조향 데이터를 **MCU에서 동일 타임라인으로 통합 수집(100 Hz)** 하고, UART로 Raspberry Pi에 넘겨 Hotspot Wi‑Fi로 서버에 로깅·실시간 표시·카메라 녹화까지 연결한다.  
> (현재 구현 보드는 STM32F407이며, **시간 동기화가 목적이라 Teensy 등 다른 MCU로 대체 가능** — 자세한 내용은 §5.2)
...
### 5.1 MCU 중앙 통합 — 핵심은 “시간 동기화”

이 프로젝트에서 MCU(현재 구현은 STM32F407)를 둔 **가장 큰 이유**는 브랜드가 아니라 **시간 동기화**입니다.

- 리니어·가속도·인버터/BMS CAN·조향 CAN을 **서로 다른 장치·주기**로 받으면, 나중에 맞춰도 샘플 시각이 어긋나기 쉽습니다.
- 그래서 **한 MCU의 틱(`stm_ms` / `HAL_GetTick` 계열)으로 모든 채널을 같은 타임라인에 찍어** Pi·서버로 넘기는 구조를 택했습니다.
- Pi는 네트워크·카메라·업로드를 담당하고, **정밀 주기 샘플링·CAN 집약은 MCU**에 두는 역할 분리입니다.

| | 내용 |
|--|------|
| **선택 이유** | 서스펜션·구동·조향을 **동일 타임스탬프**로 맞춤 |
| **장점** | 단일 UART 스트림, 디버그·확장 용이, 실차 EMI 환경에서도 샘플 주기를 MCU가 책임 |
| **리스크** | MCU/UART가 단일 장애점, CSV가 길면 UART·무선 부하 증가 |

### 5.2 STM32를 꼭 고집할 필요는 없음

**“STM이어야만 한다”는 제약은 없습니다.**  
필요한 것은 대략 다음을 만족하는 **실시간 MCU**면 됩니다.

- ADC(리니어) + (가능하면) SPI/I2C 센서
- CAN 1~2채널 (인버터/BMS/조향)
- 안정적인 주기 타이머·타임스탬프
- Pi로 넘길 UART(또는 USB serial 등) 고속 스트림

그래서 예를 들면 아래도 **같은 역할의 중앙 계측 MCU**로 대체·재구현 가능합니다.

| 후보 | 메모 |
|------|------|
| **Teensy** (4.x 등) | 고속 클럭, USB/시리얼 편함, 커뮤니티 센서 예제 많음 |
| **다른 STM32** | 핀맵·Cube 설정만 맞으면 현재 구조와 가장 유사 |
| **ESP32** 계열 | Wi‑Fi 내장이라 Pi를 줄이는 실험도 가능하나, CAN·실시간·EMI는 설계 부담이 달라짐 |
| **기타 Cortex‑M / 보드** | ADC+CAN+UART만 되면 파이프라인 개념은 동일 |

지금 저장소 펌웨어는 **STM32F407 Discovery/Cube 기반**으로 작성된 **한 가지 구현**입니다.  
구조(센서·CAN → MCU 타임라인 → 시리얼 → Pi → 서버)만 유지하면 **Teensy 등 다른 MCU로 포팅해도 설계 의도는 그대로**입니다.

### 5.3 대안이었던 분산 전송

- 센서·카메라를 각자 올리거나 주기만 다르게 하는 방식
- 무선 부하는 나눌 수 있으나 **시각 동기·배선·프로토콜 복잡도**가 커짐 → 본 프로젝트 목표(동기 로깅)와는 덜 맞음

### 5.4 샘플링 100 Hz / UART 921600

- 200 Hz → 100 Hz: 루프·UART·업로드 여유
- 460800 ↔ 921600: 용량 vs EMI/배선 안정성 트레이드오프. 근거리면 921600 가능

### 5.5 리니어 캘리브레이션을 소프트웨어로

- 채널마다 풀전압이 다름 (하드웨어·케이블·장착)
- 코드에서 채널별 `vmax`로 100 mm 맞춤 — **배선 불량(신호/파워 교차)은 캘리로 고칠 수 없음**

### 5.6 텔레메트리 전송 방식의 다른 선택지

계측에서 “차 → 기록 PC/서버”로 데이터를 넘기는 방법은 팀이 제각각이라, **이 저장소 방식(MCU + Pi + Hotspot Wi‑Fi)이 유일한 정답은 아닙니다.**  
크게 보면 아래 세 갈래로 나뉩니다.

#### (A) 상용 데이터로거를 사서 쓰는 경우

- 완성품 로거에 센서/CAN을 꽂고 USB·자체 소프트웨어로 기록
- **장점:** 배선·펌웨어·동기 UI가 제품에 포함되어 있어 구현 부담이 작음
- **단점:** 가격, 커스텀 필드/서버 연동·실시간 웹 파이프라인은 제품 제약을 따름
- DIY 계측을 깊게 파고든 팀을 찾기 어려운 이유 중 하나가, **많은 팀이 이 경로로 가기 때문**입니다. 상용 로거 사용 자체는 문제 없고, 다만 **자작 MCU 파이프라인 노하우를 물어볼 상대는 상대적으로 적습니다.**

#### (B) 본 프로젝트 — MCU 집약 + Pi + Hotspot(Wi‑Fi/인터넷)

- MCU가 동기 샘플 → UART → Pi → Hotspot → 집 서버
- **장점:** 필드·서버·카메라를 자유롭게 확장, 비용 대비 커스터마이즈
- **단점:** 무선은 Hotspot/공유기 품질에 민감, 직접 디버깅·보안·전원 설계가 필요

#### (C) UART형 장거리 무선 모듈 — 예: **EBYTE E220-900T22S**

일부 팀은 Wi‑Fi/휴대폰 핫스팟 대신, MCU(또는 중간 보드)의 **UART에 LoRa 계열 무선 시리얼 모듈**을 달아 송수신합니다.

| 항목 | E220-900T22S 요약 ([Ebyte 제품 페이지](https://www.cdebyte.com/products/E220-900T22S)) |
|------|------|
| 종류 | LoRa 기반 **무선 시리얼(UART) 모듈** (LLCC68) |
| 주파수 | 약 **850.125 ~ 930.125 MHz** (기본 예: 873.125 MHz, 868/915 MHz 대역 운용) |
| 출력 | 최대 **22 dBm** |
| 공칭 거리 | 개활지 기준 **수 km**급 (안테나·속도·환경에 크게 좌우) |
| 공중 전송률 | 대략 **2.4 kbps ~ 62.5 kbps** (설정에 따라) — **Wi‑Fi보다 훨씬 느림** |
| 인터페이스 | TTL UART (3.3 V 로직), MCU 시리얼에 붙이는 형태 |

**어떻게 쓰이나 (개념):**

```text
[차량 MCU] --UART--> [E220 송신]  ~~~900MHz LoRa~~~  [E220 수신] --UART--> [기록 PC / 다른 MCU]
```

- Hotspot·공유기·인터넷 없이도 **차 ↔ 피트/노트북** 직통에 가깝게 쓸 수 있는 선택지
- 다만 공중 속도가 낮아 **본 프로젝트처럼 긴 CSV를 100 Hz로 풀 스트리밍**하기엔 대역이 부족할 수 있음  
  → 보통은 **샘플 주기·필드 축소, 배치/압축, 또는 로컬 SD에 풀레이트 저장 + 무선은 요약/저속** 같은 설계가 필요
- 주파수·출력은 **해당 국가 전파 규정**을 반드시 확인할 것 (무단 고출력·비허가 대역 사용 금지)

본 저장소는 **(B) Wi‑Fi/Hotspot 경로를 구현·문서화**한 것이며, **(C) E220-900T22S 같은 UART LoRa 모듈 통신도 업계·동계에서 쓰이는 다른 갈래**라는 점만 참고용으로 남깁니다.  
이 레포에 E220 드라이버/설정 코드는 포함되어 있지 않습니다.
...
- MCU 채택 이유(시간 동기화) 명시, STM 외 Teensy 등 대체 가능 안내
- 텔레메트리 대안: 상용 로거 / Wi‑Fi·Hotspot / EBYTE E220-900T22S(UART LoRa) 비교 정리
