...
async function process_log(raw) {
  ...
  for (let line of lines) {
    try {
      ...
      // parsed.timestamp 등이 있을 수 있음
      // 원하는 대로 필드 재가공
      log.push(parsed);
    } catch (e) {
      console.error("라인 파싱 실패:", line, e);
      error++;
    }
  }
  ...
  $("#data-count").text(lines.length);
  $("#error-count").text(error);
  $("#converted-count").text(lines.length - error);
  // duration 등은 실제 timestamp 필드를 활용해서 계산하거나, 필요 없으면 생략
...
function loadLogFileList() {
  $.ajax({
    ...
    success: function(files) {
      const select = $('#logfile-select');
      select.empty().append('<option value="">로그 파일 선택</option>');
      files.forEach(function(filename) {
        select.append(`<option value="${filename}">${filename}</option>`);
      });
    },
    error: function(err) {
      console.error("로그 파일 목록을 불러오지 못했습니다.", err);
      Swal.fire({
         icon: "error",
         title: "로그 파일 목록 불러오기 실패",
         text: "서버에서 로그 파일 목록을 불러오지 못했습니다."
      });
    }
  });
}

// 문서 로드 시 로그 파일 목록을 가져옵니다.
$(document).ready(function(){
  loadLogFileList();
});

// "불러오기" 버튼 클릭 이벤트
$('#load-logfile').on('click', function() {
  const filename = $('#logfile-select').val();
  if (!filename) {
    Swal.fire({
       icon: "warning",
       title: "로그 파일 선택",
       text: "불러올 로그 파일을 선택해 주세요."
    });
    return;
  }
  $.ajax({
     url: '/api/logfile?name=' + encodeURIComponent(filename),
     method: 'GET',
     dataType: 'text',
     success: function(data) {
        // 서버에서 반환한 텍스트 데이터를 Blob 객체로 변환하여 process_log() 호출
        const raw = new Blob([data], { type: 'application/octet-stream' });
        process_log(raw);
     },
     error: function(err) {
